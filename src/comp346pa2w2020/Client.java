...
public class Client extends Thread {
    ...
    static boolean receiveTerm = false;                 // statically tracks if receive client terminated     // MYDEBUG: I added these in PA1, might need?
    static boolean sendTerm = false;                 // statically tracks if send client terminated     // MYDEBUG: I added these in PA1, might need?
     ...
     */
     ...
     { 
       ...
       { 
           ...
           String cip = Network.getClientIP();              // MYDEBUG: changed from 'objNetwork' to 'Network', same happened everywhere Network referenced in this file
     ...
     */
     public void readTransactions()
     {
        ...
        try
        {
        	inputStream = new Scanner(new FileInputStream("transaction2.txt"));             // MYDEBUG: changed 'transaction.txt' to 'transaction2.txt', same below in S.O.Ps in this function
        }
        ...
        {
            try
            {   transaction[i] = new Transactions();                            // MYDEBUG: interesting, create new Transactions() not in a lock. Implement lock here?
     ...
     */
     public void sendTransactions()
     {
         ...
         {  
        	 ...
        	 while ((Network.getInBufferStatus().equals("full")) && (!Network.getServerConnectionStatus().equals("disconnected")))
        	{
    ...
    public void run()
    {
        ...
        else if (getClientOperation().equals("sending"))
        {
        ...
        if (sendTerm && receiveTerm) {
            Network.disconnect(Network.getClientIP());
            // Client only disconnects once both client send and receive transactions terminate. Checks statically.
        }
    }
}
