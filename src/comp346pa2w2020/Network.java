...
public class Network extends Thread {
    ...
    private static Semaphore mutex;      // Declaration of Semaphore variable
    private static Semaphore mutex2;      // Declaration of Semaphore variable
     ...
     * @param
     */
     Network( )             // MYDEBUG: old: 'Network(String context)'
      {
          mutex = new Semaphore(1);      // Instantiation of Semaphore variable
          mutex2 = new Semaphore(1);      // Instantiation of Semaphore variable
      ...
      }                                 // MYDEBUG: missing network components for client/server
     ...
     public static boolean receive(Transactions outPacket)
    {
        ...
        // MYDEBUG: Start of CS (Mutex). I used a synchronized statement instead of method because the deposit, withdrawal and query methods require different code to be executed, so they can't share a synchronized method
        try {                       // Must run mutex in try/catch
            mutex2.acquire();        // Acquires mutex lock
            ...
            mutex2.release();        // Releases mutex lock
        } catch (Exception e) {
            e.printStackTrace();    // Prints stack trace in case of exception
        }
    ...
    {	
    	...
    	while (true)
    	{
            ...
            System.exit(0);              // End network thread since client & server both disconnected.
    	}
    }
}
